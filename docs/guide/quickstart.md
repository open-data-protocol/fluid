...
A FLUID contract is **one YAML file**. This page shows the smallest file that validates against FLUID **0.7.4**, explains each required block, and shows how to validate it.
...
|---|---|---|
| `fluidVersion` | ✅ | Which contract version this file targets. Use `"0.7.4"` for the latest schema. |
...
> **0.7.4 note.** `exposes[]` also accepts an optional `mcp` block for the MCP output-port gateway, `binding.platform` adds `postgres`, and `binding.format` adds `postgres_table` / `athena_table` / `glue_table`. When an expose carries an `mcp` block, its `policy.agentPolicy` is enforced at runtime at the MCP gateway (the `agentPolicy` shape is unchanged). 0.7.4 is **additive and fully backward-compatible** — every valid 0.7.3 contract still validates, so bumping `fluidVersion` is all it takes.

## Validate it

Validate any `.fluid.yml` against the published JSON Schema for 0.7.4:
