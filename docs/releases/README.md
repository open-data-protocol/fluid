...
The 0.7.1 → 0.7.4 line is strictly **additive**: bump `fluidVersion` and every prior contract still validates.

**0.7.4 continues that streak.** It only adds optional fields and enum values, so every valid 0.7.3 contract validates as 0.7.4 unchanged. See [What's New in 0.7.4](/fluid/releases/0.7.4#backward-compatibility-fully-additive) for the details.
